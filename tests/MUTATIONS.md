...
| `bearing` | replace the heal's capture-and-restore with Leaflet's own compensation (`invalidateSize({debounceMoveend:true})`, no `setView`) | "the point that was in the middle is still in the middle" fails with 189 px drift, want 0 ±2. Note the halves do NOT bite separately: with the restore in place the `pan` option is irrelevant (measured), which is the point of doing it this way |
