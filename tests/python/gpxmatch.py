...
SLOW: ~2-4 minutes, because it runs the real matcher over ten tours' worth of region geometry. Deliberately
NOT `@always` — it is selected only when the matcher or its own harness changes, which is exactly when its
...
def run(t):
    ...
    t.ok("no absolute machine path is baked into it", "D:\\Trailmap" not in src,
         "D:\\Trailmap" in src, False)
    ...
    p = subprocess.run([sys.executable, HARNESS], capture_output=True, text=True, cwd=ROOT, timeout=900)
